...
object Main extends IOApp {
  ...
  def gameLoop(b: Board): IO[Unit] = {
    def validateInput(board: Board, input: String): Either[InputError, (Int, Int, Int, Int)] = {
      val inputPattern = """^(\d+)\s+(\d+)\s*,\s*(\d+)\s+(\d+)\s*$""".r
      input match {
    ...
    def readInput(board: Board): IO[Board] = {
      for {
        ...
        updatedBoard <- validateInput(board, input) match {
          case Left(InvalidFormatError) => IO.println("Invalid Input! Please use the format \"<x1> <y1>, <x2> <y2>\"") *> (readInput(board))
          case Left(OutOfBoundsError) => IO.println("Invalid Coordinates! Please enter values within the board's limits") *> (readInput(board))
          case Left(InvalidMatchError) => IO.println("Invalid Match! The tiles you selected cannot be linked. Please check the path and try again.") *> (readInput(board))
    ...
    for {
      ...
      _ <- IO.println("How to play: Enter coordinates of two tiles to match in the following format: <x1> <y1>,<x2> <y2>")
