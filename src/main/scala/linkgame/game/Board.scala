...
object Board {
  ...
  final case class Path(points: List[Coordinate]) // Coordinates of start, end and turned points
  ...
  def getValidPath(board: Board, p1: (Int, Int), p2: (Int, Int)): Option[Path] = {
    ...
    def matchStraightLine(a: (Int, Int) = p1, b: (Int, Int) = p2): Option[List[(Int, Int)]] = {
      val (x1, y1) = a
      val (x2, y2) = b
      if (x1 == x2) {
        if (((y1.min(y2) + 1) until y1.max(y2)).forall(y => isEmpty((x1, y)))) Some(List(a, b)) else None
      } else if (y1 == y2) {
        if (((x1.min(x2) + 1) until x1.max(x2)).forall(x => isEmpty((x, y1)))) Some(List(a, b)) else None
      } else {
        None
      }
    }

    def matchOneTurnLine(a: (Int, Int) = p1, b: (Int, Int) = p2): Option[List[(Int, Int)]] = {
      // the turn must occur at point c = (x1, y2) or c = (x2, y1)
      // a -> c -> b
      val c = (a._1, b._2)
      val d = (b._1, a._2)
    ...
    def matchTwoTurnLine(a: (Int, Int) = p1, b: (Int, Int) = p2): Option[List[(Int, Int)]] = {
      ...
      candidates.distinct.view.flatMap { c =>
        if (!isEmpty(c)) None
          ...
          matchStraightLine(a, c).flatMap { path1 =>
    ...
    if (basicCondition()) {
      matchStraightLine().orElse(matchOneTurnLine()).orElse(matchTwoTurnLine()).map { path =>
        Path(path.map { case (x, y) => Coordinate(x, y) })
      }
    } else {
      None
    }
